...
/**
 * sub_jump_list - value in a sorted list of integers using the Jump search algorithm.
 *
...
int value, int step, int flag_next_buffer)
{
	...
	else if (buffer->n < value && buffer->next)
	{
		...
		for (iter = 0, next_buffer = buffer; iter < step && next_buffer->next; iter++)
		...
		return sub_jump_list(next_buffer, buffer, size-1, value, step, flag_next_buffer);
	}
	else if (buffer->n >= value)
	{
		print_array(buffer->index, prev_list->index, 0, 0);
		return sub_jump_list(prev_list, prev_list, size, value, 1, flag_next_buffer);
	}
	return (NULL);
}
